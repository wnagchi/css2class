...
class ClassParser {
  ...
  parseClassOptimized(htmlStr) {
    ...
    try {
      let match;
      const classAttrRegex = new RegExp(compiledRegex.classAttr.source, compiledRegex.classAttr.flags);
  ...
  isKnownCSSProperty(property) {
    const knownProperties = [
      ...
      'text', 'font', 'leading', 'tracking', 'spacing',
      'top', 'right', 'bottom', 'left', 'inset',
      'rounded', 'border', 'bordert', 'borderr', 'borderb', 'borderl',
      'opacity', 'op', 'transition', 'gap', 'size', 'b_r'
    ];
  ...
  getCSSPropertyMapping(property) {
    const mappings = {
      ...
      'text': 'font-size', 'font': 'font-weight', 'leading': 'line-height', 'tracking': 'letter-spacing',
      'top': 'top', 'right': 'right', 'bottom': 'bottom', 'left': 'left',
      'rounded': 'border-radius', 'border': 'border-width', 'opacity': 'opacity', 'gap': 'gap'
    };
  ...
  analyzeUnit(property, value) {
    ...
    if (detectedUnit && recommendedUnit && detectedUnit !== recommendedUnit) {
      ...
      analysis.suggestions.push(`考虑使用 ${property}-${value.replace(detectedUnit, '')}${recommendedUnit ? recommendedUnit : ''}`);
    }
  ...
  validateParseResult(result) {
    ...
    const duplicates = allClasses.filter((className, index) => allClasses.indexOf(className) !== index);
  ...
  debugParse(htmlStr, maxLength = 1000) {
    const truncatedHtml = htmlStr.length > maxLength 
      ? htmlStr.substring(0, maxLength) + '...' 
      : htmlStr;
