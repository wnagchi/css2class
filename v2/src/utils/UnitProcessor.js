class UnitProcessor {
  ...
  isSpecialProperty(property, value) {
    return this.unitlessProperties.has(property) || 
           property === 'opacity' || 
  ...
  getPropertyUnit(property) {
    ...
    if (property.includes('top') || property.includes('right') || 
  ...
  processRuleConfig(ruleConfig, value) {
    ...
    if (Array.isArray(properties)) {
      return properties.map(property => 
        this.parseValue(value, property, defaultUnit)
      );
    }
  ...
  createCSSValue(property, value, unit = null) {
    ...
    if (Array.isArray(processedValue)) {
      return processedValue.map((val, index) => {
