class ClassChangeTracker {
  ...
  getChangeStatistics(changes) {
    return {
      ...
      changeRatio: changes.total.old > 0 ? 
        ((changes.added.length + changes.removed.length) / changes.total.old * 100).toFixed(2) + '%' : '0%',
      categories: Object.keys(changes.categories).filter(key => 
        changes.categories[key].length > 0
      ),
      reasons: Object.keys(changes.reasons).filter(key => 
        changes.reasons[key]
      )
    };
  }
