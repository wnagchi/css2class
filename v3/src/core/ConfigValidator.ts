...
import { Config, ValidationReport, AutoFixReport, Recommendation, ValidationSummary } from '../../types';
...
class ConfigValidator {
  ...
  // 验证系统配置
  private validateSystemConfig(config: Config): void {
    const system = config.system as SystemConfig || {};
    ...
    // 验证单位转换比例
    const conversion = parseFloat(String(system.unitConversion));
  ...
  // 验证原子化规则
  private validateAtomicRules(config: Config): void {
    const atomicRules = config.atomicRules as RuleCategory || {};
  ...
  // 验证规则类别
  private validateRuleCategory(categoryName: string, categoryRules: RuleCategory): void {
  ...
  // 验证单个规则
  private validateRule(ruleName: string, ruleConfig: RuleConfig, categoryName: string): void {
  ...
  // 验证兼容性配置
  private validateCompatibilityConfig(config: Config): void {
    const compatibility = config.compatibility as CompatibilityConfig || {};
  ...
  // 验证legacy配置
  private validateLegacyConfig(legacy: LegacyConfig): void {
    // 验证cssName
  ...
  // 检测配置冲突
  private detectConflicts(config: Config): void {
  ...
  // 检测text属性冲突
  private detectTextConflicts(config: Config): string[] {
    const conflicts: string[] = [];
  ...
  // 检测重复定义
  private detectDuplicateDefinitions(config: Config): string[] {
    const conflicts: string[] = [];
    const allDefinitions = new Set<string>();

    // 收集所有定义
    const collectDefinitions = (obj: any, prefix = ''): void => {
  ...
  // 检查单位一致性
  private checkUnitConsistency(config: Config): void {
    // 检查基础单位设置
    ...
    Object.entries(atomicClassMap).forEach(([key, value]) => {
      if (value && typeof value === 'object' && (value as any).unit) {
        // 检查是否与基础单位一致
        if ((value as any).unit !== baseUnit && (value as any).unit !== '') {
          this.warnings.push(
            `Unit inconsistency: ${key} uses ${(value as any).unit} instead of ${baseUnit}`
          );
        }
      }
    });
  ...
  // 自动修复配置
  autoFix(config: Config): AutoFixResult {
  ...
  // 生成配置报告
  generateReport(config: Config): ConfigReport {
  ...
  // 生成优化建议
  private generateRecommendations(config: Config): string[] {
    ...
    if (config.atomicClassMap) {
      ...
      Object.entries(config.atomicClassMap).forEach(([key, value]) => {
        if (value && typeof value === 'object' && (value as any).unit && (value as any).unit !== baseUnit) {
