...
/**
 * Class2CSS 快速启动脚本 - TypeScript版本
 * 使用方法: npm run dev 或 node run.js
 */

import Class2CSS from './src/index';
...
async function run(): Promise<void> {
  try {
    ...
    eventBus.on('parser:completed', (stats) => {
      const totalCount = stats && (stats as any).totalCount ? (stats as any).totalCount : 0;
      console.log(`🔍 解析完成: 发现 ${totalCount} 个类名`);
    });

    eventBus.on('generator:dynamic:completed', (stats) => {
      const generatedCount = stats && (stats as any).generatedCount ? (stats as any).generatedCount : 0;
      console.log(`🎨 动态CSS生成完成: ${generatedCount} 个类`);
    });
