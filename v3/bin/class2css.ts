...
// 检查配置文件是否存在
function checkConfig(configPath: string): string {
  const absolutePath = path.resolve(configPath);

  // 尝试.js和.ts两种配置文件
  const jsConfigPath = absolutePath.replace(/\.ts$/, '.js');
  const tsConfigPath = absolutePath.replace(/\.js$/, '.ts');
  ...
  if (fs.existsSync(tsConfigPath)) {
    ...
    console.error('Please create a class2css.config.js or class2css.config.ts file.');
    process.exit(1);
  }
}
